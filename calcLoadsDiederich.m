function Loads = calcLoadsDiederich(AC, wing, isa)
    ...
    CL_w = 1.05*CL; % Placeholder for wing lift coefficient, update with actual value
    ...
    F = (2*pi*wing.ARe)/(cl_alpha*cosd(wing.lambda_e_25));
    ...
    for i = 1:length(eta)
        f_lambda = (4/pi)*sqrt(1-eta(i)^2); % approximate for low sweep, or implement chart lookup
        L_additional(i) = C1*(c(i)/wing.c_bar) + C2*(4/pi)*sqrt(1-eta(i)^2) + C3*f_lambda;
